...
Experiments to test if changing the game complexity (Normal og Hard Tetris) can be used for tuning the CMA-ES and the Cross-entropy method configurations in later experiments.\\
...
Experiments to test Bertsekas and Dellacherie featuresets in regards to achieved score. This will tell if a different feature set affects the algorithms performance. 
...
The Cross-entropy method for the Tetris problem uses 10 \% Parent selection as standard. However, we will also test 25\% and 50\% Parent selection derived from configurations in CMA-ES.\\
...
Experiments to determine the optimal number of games played per agent for best resulting score at lowest evaluation cost.
...
\end{tabular}
\caption{Results of CMA-ES with various initial step-size \label{CMAInitialSigmaConfigTestAppendix}}
\end{figure}
...
Experiments finding the best performing configuration of population and parent size with
...
\end{tabular}
\caption{Full experiments overview - more intuitive format \label{SuperCMAExperimentAppendix}}
\end{table}
