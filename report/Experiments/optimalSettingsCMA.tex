...
\end{itemize}
By default, CMA-ES uses Super Linear recombination. However, Tetris is a problem
with multiple local optimums and heavy noise in its search space. This means, though a vector may be the best in its generation, it could be a nearby local optimum. Therefore, Super Linear recombination may not be the optimal recombination type for the Tetris problem.\\
...
performance of an agent. The Linear and Super Linear recombination types will value the better agents higher.
...
when we tuned the Cross-entropy method (see section \ref{optimalsettingsce}). However, we use a population
size of 12 instead of 10, because a population size of 12 is the stock setting for Shark CMA-ES \citep{shark08}.\\
